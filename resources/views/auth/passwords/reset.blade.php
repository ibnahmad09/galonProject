...
<div class="w-full max-w-md p-8 bg-white/80 rounded-2xl shadow-xl backdrop-blur-md">
    <div class="flex flex-col items-center mb-6">
        ...
        <h2 class="text-2xl font-bold text-blue-700 mb-1">Reset Password</h2>
        <p class="text-blue-500 text-sm text-center">Masukkan password baru untuk akun kamu.</p>
    </div>
    <form method="POST" action="{{ route('password.update') }}" class="space-y-5">
        ...
        <div>
            ...
            <input id="email" type="email" name="email" value="{{ $email ?? old('email') }}" required autofocus class="w-full px-4 py-2 border border-blue-200 rounded-lg focus:ring-2 focus:ring-blue-400 focus:outline-none">
            @error('email')
                <span class="text-red-500 text-xs">{{ $message }}</span>
            @enderror
        </div>
        <div>
            <label for="password" class="block text-blue-800 font-semibold mb-1">Password Baru</label>
        ...
        <button type="submit" class="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow transition">Reset Password</button>
    </form>
</div>
@endsection
